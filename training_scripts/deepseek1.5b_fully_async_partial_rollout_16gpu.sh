...
RESUME_FROM_PATH=${RESUME_FROM_PATH:-""}  # Example: "ckpts/DeepSeek1.5B/DeepSeek1.5B-FullyAsync-PartialRollout-16gpu-20260110_123456/global_step_100"
...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  # Try to find existing checkpoint directory
    LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/DeepSeek1.5B -name "DeepSeek1.5B-FullyAsync-PartialRollout-16gpu-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
    echo "Auto-found existing checkpoint directory: $CKPTS_DIR"
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-FullyAsync-PartialRollout-16gpu-${TIMESTAMP}
...
LOG_FILE="${LOG_DIR}/deepseek1.5b_fully_async_partial_rollout_16gpu_${TIMESTAMP}.log"
...
# Rollout: 4 GPUs per node = 8 GPUs total (2 nodes * 4 GPUs)
# Training: 4 GPUs per node = 8 GPUs total (2 nodes * 4 GPUs)
export NNODES=2
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.experiment_name="DeepSeek1.5B-FullyAsync-PartialRollout-16gpu" \
