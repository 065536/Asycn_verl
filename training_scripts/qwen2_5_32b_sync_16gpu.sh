#!/bin/bash
set -xeuo pipefail
# Qwen2.5-32B GRPO/DAPO on 16 GPUs
# Copied from qwen2_5_32b_sync_32gpu.sh, keeping algorithm params the same.
...
# Default auto resume: finds latest Qwen2.5-32B-Sync-16gpu-* and continues from last checkpoint
RESUME_MODE=${RESUME_MODE:-"disable"}
RESUME_FROM_PATH=${RESUME_FROM_PATH:-""}
USER_CKPTS_DIR=${CKPTS_DIR:-""}
# 保存的最大 checkpoint 数量，只保留最新 1 个
...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/Qwen2.5-32B -name "Qwen2.5-32B-Sync-16gpu-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
    echo "Auto-found existing checkpoint directory: $CKPTS_DIR"
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/Qwen2.5-32B/Qwen2.5-32B-Sync-16gpu-${TIMESTAMP}
...
LOG_FILE="${LOG_DIR}/qwen2_5_32b_sync_16gpu_${TIMESTAMP}.log"
...
# Data: 17k-sampled train, DeepScaler test sets (override via TRAIN_FILE/TEST_FILES)
...
# 16 GPUs: 2 nodes x 8 GPUs
export NNODES=2
export NGPUS_PER_NODE=8
export CKPTS_DIR="$CKPTS_DIR"

# DAPO params (keep same as 32gpu)
...
echo "Total: ${NNODES} nodes * ${NGPUS_PER_NODE} GPUs/node = $((NNODES * NGPUS_PER_NODE)) GPUs total"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="Qwen2.5-32B-Sync-16gpu-DAPO" \
