#!/bin/bash
set -xeuo pipefail
# Qwen2.5-32B GRPO/DAPO on 32 GPUs
# Ray setup from deepseek7b_sync_32gpu.sh, params from DAPO recipe
...
# Default auto resume: finds latest Qwen2.5-32B-Sync-32gpu-* and continues from last checkpoint
...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/Qwen2.5-32B -name "Qwen2.5-32B-Sync-32gpu-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
    echo "Auto-found existing checkpoint directory: $CKPTS_DIR"
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/Qwen2.5-32B/Qwen2.5-32B-Sync-32gpu-${TIMESTAMP}
...
LOG_FILE="${LOG_DIR}/qwen2_5_32b_sync_32gpu_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="Qwen2.5-32B-Sync-32gpu-DAPO" \
