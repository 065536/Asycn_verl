...
RESUME_FROM_PATH=${RESUME_FROM_PATH:-""}  # Example: "ckpts/DeepSeek1.5B/DeepSeek1.5B-FullyAsync-PartialRollout-8gpu-20260110_123456/global_step_100"
...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  # Try to find existing checkpoint directory
    LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/DeepSeek1.5B -name "DeepSeek1.5B-FullyAsync-PartialRollout-8gpu-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
    echo "Auto-found existing checkpoint directory: $CKPTS_DIR"
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-FullyAsync-PartialRollout-8gpu-${TIMESTAMP}
...
LOG_FILE="${LOG_DIR}/deepseek1.5b_fully_async_partial_rollout_8gpu_v4_${TIMESTAMP}.log"
...
# Hydra 的 file://verl/trainer/config 是相对 cwd 的，需在项目父目录下才能解析到 verl2619/verl/trainer/config
cd "${VERL_ROOT}/.."
...
# 1 node * 8 GPUs = 8 GPUs
# For fully-async: allocate resources for rollout and training (1:1 allocation)
# Rollout: 4 GPUs per node = 4 GPUs total (1 node * 4 GPUs)
# Training: 4 GPUs per node = 4 GPUs total (1 node * 4 GPUs)
export NNODES=1
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    actor_rollout_ref.actor.grad_clip=10000.0 \
    ...
    trainer.experiment_name="DeepSeek1.5B-FA-8gpu-lr-1e-5" \
