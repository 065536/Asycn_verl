...
RESUME_FROM_PATH=${RESUME_FROM_PATH:-""}  # Example: "ckpts/DeepSeek7B/DeepSeek7B-Sync-32gpu-20260110_123456/global_step_100"
...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  echo "Using user-specified checkpoint directory: $CKPTS_DIR"
    ...
    LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/DeepSeek7B -name "DeepSeek7B-Sync-32gpu-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
    echo "Auto-found existing checkpoint directory: $CKPTS_DIR"
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek7B/DeepSeek7B-Sync-32gpu-${TIMESTAMP}
...
LOG_FILE="${LOG_DIR}/deepseek7b_sync_32gpu_${TIMESTAMP}.log"
...
ppo_max_token_per_gpu=$((max_prompt_length + max_response_length + 300))
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    +reward_model.reward_kwargs.max_resp_len=$((max_prompt_length + max_response_length)) \
    trainer.logger='["console","swanlab"]' \
    trainer.project_name="DeepSeek7B" \
    trainer.experiment_name="DeepSeek7B-Sync-32gpu" \
