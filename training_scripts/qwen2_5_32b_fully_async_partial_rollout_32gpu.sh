#!/bin/bash
set -xeuo pipefail
# Qwen2.5-32B Fully Async GRPO/DAPO on 32 GPUs (With Partial Rollout)
# Based on qwen2_5_32b_fully_async_no_partial_rollout_32gpu.sh, with partial_rollout=True

VERL_ROOT=/data/250010176/codes/verl/verl
DATA_ROOT=/data/250010176/data
MODEL_PATH=/data/250010176/codes/models/Qwen2.5-32B
...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/Qwen2.5-32B -name "Qwen2.5-32B-FullyAsync-PartialRollout-32gpu-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    RESUMING=true
    echo "Auto-found existing checkpoint directory: $CKPTS_DIR"
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/Qwen2.5-32B/Qwen2.5-32B-FullyAsync-PartialRollout-32gpu-${TIMESTAMP}
    echo "No existing checkpoint found, creating new directory: $CKPTS_DIR"
  fi
else
  CKPTS_DIR=${VERL_ROOT}/ckpts/Qwen2.5-32B/Qwen2.5-32B-FullyAsync-PartialRollout-32gpu-${TIMESTAMP}
  echo "Resume disabled, creating checkpoint directory: $CKPTS_DIR"
fi

LOG_DIR=${VERL_ROOT}/logs
if [ "$RESUMING" = true ]; then
  LOG_TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
  LOG_FILE="${LOG_DIR}/qwen2_5_32b_fully_async_partial_rollout_32gpu_${LOG_TIMESTAMP}.log"
  echo "Resuming training - creating new log file: $LOG_FILE"
else
  LOG_FILE="${LOG_DIR}/qwen2_5_32b_fully_async_partial_rollout_32gpu_${TIMESTAMP}.log"
fi
...
# DAPO params (from sync script)
max_prompt_length=2048
max_response_length=20480
n_resp_per_prompt=16
ppo_max_token_per_gpu=16384

# 32B: gen_tp=4, sp_size=1
gen_tp=4
sp_size=1
...
test_freq=10
staleness_threshold=0.5       # Partial rollout: higher staleness for efficiency
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.project_name="Qwen2.5-32B" \
    trainer.experiment_name="Qwen2.5-32B-FullyAsync-PartialRollout-32gpu-DAPO" \
