...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  echo "Using user-specified checkpoint directory: $CKPTS_DIR"
    ...
    LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/DeepSeek1.5B -name "DeepSeek1.5B-FullyAsync-PartialRollout-8gpu-ent0.01-lr1e-6-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
    echo "Auto-found existing checkpoint directory: $CKPTS_DIR"
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-FullyAsync-PartialRollout-8gpu-ent0.01-lr1e-6-${TIMESTAMP}
...
LOG_FILE="${LOG_DIR}/async_partial_entropy_bonus_lr1e-6_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.experiment_name="deepseek1.5b_fa_partial_8gpu_ent0.01_lr1e-6" \
