...
# Adaptive entropy-coeff defaults (can be overridden by env)
BASE_ENTROPY_COEFF=${BASE_ENTROPY_COEFF:-0.001}
ENTROPY_COEFF_MIN_RATIO=${ENTROPY_COEFF_MIN_RATIO:-0.5}
ENTROPY_COEFF_MAX_RATIO=${ENTROPY_COEFF_MAX_RATIO:-5.0}
ENTROPY_COEFF_REF_MODE=${ENTROPY_COEFF_REF_MODE:-initial}
ENTROPY_COEFF_EMA_BETA=${ENTROPY_COEFF_EMA_BETA:-0.95}
...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/DeepSeek1.5B -name "DeepSeek1.5B-Sync-8gpu-adaptive-entcoeff-lr1e-5-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-Sync-8gpu-adaptive-entcoeff-lr1e-5-${TIMESTAMP}
  fi
else
  CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-Sync-8gpu-adaptive-entcoeff-lr1e-5-${TIMESTAMP}
fi
...
LOG_FILE="${LOG_DIR}/sync_adaptive_entropy_coeff_lr1e-5_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  [ "$ready" -ne 1 ] && echo "Ray head failed to start" && exit 1
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.actor.entropy_coeff="${BASE_ENTROPY_COEFF}" \
    actor_rollout_ref.actor.entropy_bonus_mode=uniform \
    actor_rollout_ref.actor.entropy_adaptive_coeff_enabled=true \
    actor_rollout_ref.actor.entropy_adaptive_coeff_reference_mode="${ENTROPY_COEFF_REF_MODE}" \
    actor_rollout_ref.actor.entropy_adaptive_coeff_min_ratio="${ENTROPY_COEFF_MIN_RATIO}" \
    actor_rollout_ref.actor.entropy_adaptive_coeff_max_ratio="${ENTROPY_COEFF_MAX_RATIO}" \
    actor_rollout_ref.actor.entropy_adaptive_coeff_ema_beta="${ENTROPY_COEFF_EMA_BETA}" \
    ...
    trainer.experiment_name="deepseek1.5b_sync_8gpu_adaptive_entcoeff_lr1e-5" \
