...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  echo "Using user-specified checkpoint directory: $CKPTS_DIR"
    ...
    LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/DeepSeek1.5B -name "DeepSeek1.5B-FullyAsync-PartialRollout-8gpu-combined-ours-lr1e-5-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
    echo "Auto-found existing checkpoint directory: $CKPTS_DIR"
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-FullyAsync-PartialRollout-8gpu-combined-ours-lr1e-5-${TIMESTAMP}
...
LOG_FILE="${LOG_DIR}/async_partial_adaptive_lr_advvar_lr1e-5_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    actor_rollout_ref.actor.optim.lr_scheduler_type=entropy_adaptive \
    actor_rollout_ref.actor.optim.entropy_adaptive_min_ratio=0.1 \
    actor_rollout_ref.actor.ppo_mini_batch_size=${train_prompt_mini_bsz} \
    actor_rollout_ref.actor.fsdp_config.param_offload=True \
    actor_rollout_ref.actor.fsdp_config.optimizer_offload=True \
    actor_rollout_ref.actor.entropy_coeff=0.01 \
    actor_rollout_ref.actor.entropy_bonus_mode=advantage_variance \
    ...
    trainer.experiment_name="deepseek1.5b_fa_partial_8gpu_combined_ours_lr1e-5" \
