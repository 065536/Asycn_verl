...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/DeepSeek1.5B -name "DeepSeek1.5B-Sync-8gpu-entadapt-initial-lr2e-5-min0.05-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-Sync-8gpu-entadapt-initial-lr2e-5-min0.05-${TIMESTAMP}
  fi
else
  CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-Sync-8gpu-entadapt-initial-lr2e-5-min0.05-${TIMESTAMP}
fi
...
LOG_FILE="${LOG_DIR}/sync_entropy_adaptive_initial_lr2e-5_min0.05_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.actor.optim.entropy_adaptive_min_ratio=0.05 \
    ...
    trainer.experiment_name="deepseek1.5b_sync_8gpu_entadapt_initial_lr2e-5_min0.05" \
