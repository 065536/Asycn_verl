...
# ============================================================
# Constant LR with Warmup + Decay Schedule Baseline
# ============================================================
# Environment variables:
#   SCHEDULE_TYPE — linear / cosine               (required)
#   WARMUP_STEPS  — lr_warmup_steps               (required)
#   SEED          — random seed                   (required)
#   BASE_LR       — initial LR                    (default: 1e-5)
# ============================================================

: "${SCHEDULE_TYPE:?'Set SCHEDULE_TYPE (linear or cosine)'}"
: "${WARMUP_STEPS:?'Set WARMUP_STEPS (e.g. 20)'}"
...
EXP_NAME="deepseek1.5b_sync_8gpu_${SCHEDULE_TYPE}_warmup${WARMUP_STEPS}_seed${SEED}"
CKPT_PREFIX="DeepSeek1.5B-Sync-8gpu-${SCHEDULE_TYPE}-warmup${WARMUP_STEPS}-seed${SEED}"
...
LOG_FILE="${LOG_DIR}/sync_${SCHEDULE_TYPE}_warmup${WARMUP_STEPS}_seed${SEED}_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="deepseek1.5b_sync_8gpu_warmup30_cosine_lr1e-5" \
