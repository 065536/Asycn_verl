...
# ============================================================
# Signal-Quality LR: Warmup Steps Sweep (Coordinate Descent)
# ============================================================
# Sweep signal_quality_warmup_steps with fixed base_lr.
# Only evaluates on AIME 2024 at step 300.
#
# Environment variables:
#   SQ_WARMUP     — signal_quality_warmup_steps   (required)
#   SQ_BASE_LR    — base (aggressive) LR          (default: 1e-5)
#   SQ_INFO_SRC   — reward_std_median / ...       (default: reward_std_median)
#   SQ_USE_CONC   — true/false                    (default: false)
#   SEED          — random seed                   (default: 42)
#   NCPUS         — Ray CPUs                      (default: nproc)
#
# Example:
#   SQ_WARMUP=10 SEED=0  bash sync_sq_warmup_sweep.sh
#   SQ_WARMUP=20 SEED=1  bash sync_sq_warmup_sweep.sh
#   SQ_WARMUP=40 SEED=42 bash sync_sq_warmup_sweep.sh
# ============================================================

: "${SQ_WARMUP:?'Set SQ_WARMUP (e.g. 10, 20, 40)'}"

SQ_BASE_LR=${SQ_BASE_LR:-"1e-5"}
SQ_TAG="sq_warmup${SQ_WARMUP}"
SQ_INFO_SRC=${SQ_INFO_SRC:-"reward_std_median"}
SQ_USE_CONC=${SQ_USE_CONC:-"false"}
SQ_CONC_SRC=${SQ_CONC_SRC:-"cv2_h"}
SQ_INFO_BETA=${SQ_INFO_BETA:-"0.9"}
SQ_INFO_QMIN=${SQ_INFO_QMIN:-"0.3"}
...
# Convert SQ_USE_CONC to Python bool
if [ "$SQ_USE_CONC" = "true" ] || [ "$SQ_USE_CONC" = "True" ]; then
  USE_CONC_PY="True"
else
  USE_CONC_PY="False"
fi
...
echo "HEAD_IP=$HEAD_IP IS_HEAD=$IS_HEAD SQ_BASE_LR=$SQ_BASE_LR SQ_TAG=$SQ_TAG SEED=$SEED USE_CONC=$SQ_USE_CONC"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    ++actor_rollout_ref.actor.optim.lr=${SQ_BASE_LR} \
    ++actor_rollout_ref.actor.optim.lr_warmup_steps=10 \
    ++actor_rollout_ref.actor.optim.weight_decay=0.1 \
    ++actor_rollout_ref.actor.optim.lr_scheduler_type=signal_quality \
    ++actor_rollout_ref.actor.optim.signal_quality_info_source=${SQ_INFO_SRC} \
    ++actor_rollout_ref.actor.optim.signal_quality_info_ema_beta=${SQ_INFO_BETA} \
    ++actor_rollout_ref.actor.optim.signal_quality_info_q_min=${SQ_INFO_QMIN} \
    ++actor_rollout_ref.actor.optim.signal_quality_warmup_steps=${SQ_WARMUP} \
    ++actor_rollout_ref.actor.optim.signal_quality_use_concentration=${USE_CONC_PY} \
    ++actor_rollout_ref.actor.optim.signal_quality_conc_source=${SQ_CONC_SRC} \
    ...
    trainer.project_name="deepseek1.5b_warmup_sweep" \
  ...
  echo "Signal-quality LR run completed: SQ_TAG=$SQ_TAG SEED=$SEED"
  $RAY stop || true
  exit 0
else
