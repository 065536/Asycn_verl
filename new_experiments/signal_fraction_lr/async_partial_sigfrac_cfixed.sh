...
# Async Phase 1: c_fixed sweep for signal-fraction LR.
#
# Goal:
# - Calibrate the signal-fraction scale under fully async partial rollout.
...
# - BASE_LR=1.25e-5  -> c_fixed ~= 2.5e-4
...
EXP_NAME_BASE=deepseek1.5b_fa_partial_8gpu_sigfrac_cfixed_lr${LR_TAG}
EXP_NAME=${EXP_NAME:-"${EXP_NAME_BASE}_seed${SEED}"}
CKPT_PREFIX=DeepSeek1.5B-FullyAsync-PartialRollout-8gpu-sigfrac-cfixed-lr${LR_TAG}-seed${SEED}
...
RUN_ID="${EXP_NAME}_${TIMESTAMP}_pid$$"

LOG_DIR=${VERL_ROOT}/logs
LOG_FILE="${LOG_DIR}/async_partial_sigfrac_cfixed_lr${LR_TAG}_seed${SEED}_${TIMESTAMP}.log"
HYDRA_DIR="${LOG_DIR}/hydra/${RUN_ID}"
LOCK_FILE="${LOG_DIR}/async_partial_sigfrac_cfixed_8gpu.lock"
mkdir -p "$CKPTS_DIR" "$LOG_DIR" "$HYDRA_DIR"
exec 9>"$LOCK_FILE"
echo "Waiting for async 8GPU sweep lock: $LOCK_FILE"
flock 9
echo "Acquired async 8GPU sweep lock: $LOCK_FILE"
...
# Data: 17k-sampled train, DeepScaler test sets (same as sync)
...
# Fully async specific parameters: match existing async_partial baselines.
rollout_mode="async"
rollout_name="vllm"
...
# Fully async requires the new separated TrainingWorker path. Signal-fraction
# support is implemented in the new FSDP engine for this async sweep.
...
echo "Async signal-fraction cfixed sweep: BASE_LR=${BASE_LR}, LR_TAG=${LR_TAG}, SEED=${SEED}"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  export RAY_ADDRESS="$HEAD_IP:6379"
    ...
    ++actor_rollout_ref.actor.ppo_max_token_len_per_gpu=$((1024+8192+600)) \
    ++actor_rollout_ref.ref.log_prob_max_token_len_per_gpu=$((1024+8192+600)) \
    ++actor_rollout_ref.rollout.log_prob_max_token_len_per_gpu=$((1024+8192+600)) \
