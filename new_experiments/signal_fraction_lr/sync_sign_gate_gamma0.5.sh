...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
    ...
    LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/DeepSeek1.5B -name "DeepSeek1.5B-Sync-8gpu-sign-gate-meanmatch-gamma0.5-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-Sync-8gpu-sign-gate-meanmatch-gamma0.5-${TIMESTAMP}
  fi
else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-Sync-8gpu-sign-gate-meanmatch-gamma0.5-${TIMESTAMP}
fi
...
LOG_FILE="${LOG_DIR}/sync_sign_gate_meanmatch_gamma0.5_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="deepseek1.5b_sync_8gpu_sign_gate_meanmatch_gamma0.5" \
