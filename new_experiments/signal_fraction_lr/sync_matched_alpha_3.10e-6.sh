...
# Baseline 1: constant LR matched to B's observed mean alpha
# Fixed LR = 3.10e-6 (near B mean alpha from 4.26 analysis, step21-300)
# Purpose: test whether B wins simply because average LR is higher than M(2.97e-6).
#
# Keep signal_fraction + sign_gate_gamma=1.0 path so code path matches M/A/B:
# same split-batch machinery, same logged metrics (including g_dot_positive).
...
SEED=${SEED:-42}
EXP_NAME_BASE=deepseek1.5b_sync_8gpu_matched_alpha_3.10e-6
EXP_NAME=${EXP_NAME:-"${EXP_NAME_BASE}_seed${SEED}"}
CKPT_PREFIX=DeepSeek1.5B-Sync-8gpu-matched-alpha-3.10e-6-seed${SEED}
...
LOG_FILE="${LOG_DIR}/sync_matched_alpha_3.10e-6_seed${SEED}_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    ++actor_rollout_ref.actor.optim.lr=3.10e-6 \
