...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  echo "Using user-specified checkpoint directory: $CKPTS_DIR"
    ...
    LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/DeepSeek1.5B -name "DeepSeek1.5B-Sync-8gpu-sigfrac-lr1e-6-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
    echo "Auto-found existing checkpoint directory: $CKPTS_DIR"
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-Sync-8gpu-sigfrac-lr1e-6-${TIMESTAMP}
...
LOG_FILE="${LOG_DIR}/sync_signal_fraction_lr1e-6_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.optim.signal_fraction_eta_c=0.1 \
    +actor_rollout_ref.actor.optim.signal_fraction_c_min=1e-8 \
    +actor_rollout_ref.actor.optim.signal_fraction_c_max=1e-3 \
    ...
    +actor_rollout_ref.actor.optim.signal_fraction_calib_frac=0.25 \
    ...
    trainer.experiment_name="deepseek1.5b_sync_8gpu_sigfrac_lr1e-6" \
