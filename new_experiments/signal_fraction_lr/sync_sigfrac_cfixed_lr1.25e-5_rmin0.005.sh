...
# Phase 2 r_min sweep: r_min=0.005
# Base: cfixed lr1.25e-5 (c_fixed=2.5e-4)
# Purpose: test whether lower r_min releases continuous shape modulation without destabilizing training
# r_min=0.005: half of current 0.01; r_hat pos mean ~0.022 → floor at ~23rd percentile of positive values
...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  echo "Using user-specified checkpoint directory: $CKPTS_DIR"
    ...
    LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/DeepSeek1.5B -name "DeepSeek1.5B-Sync-8gpu-sigfrac-cfixed-lr1.25e-5-rmin0.005-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
    echo "Auto-found existing checkpoint directory: $CKPTS_DIR"
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-Sync-8gpu-sigfrac-cfixed-lr1.25e-5-rmin0.005-${TIMESTAMP}
...
LOG_FILE="${LOG_DIR}/sync_sigfrac_cfixed_lr1.25e-5_rmin0.005_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.optim.signal_fraction_r_min=0.005 \
    ...
    trainer.experiment_name="deepseek1.5b_sync_8gpu_sigfrac_cfixed_lr1.25e-5_rmin0.005" \
