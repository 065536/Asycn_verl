...
# Baseline D3: state-independent piecewise stage-matched decay.
#
# Goal:
# - Copy B's coarse stage-level alpha schedule without using per-step alignment.
# - Test whether B's gains are explained by a hand-crafted early/mid/late schedule.
#
# Schedule source:
# - Values are pooled from 4.26 B (sigfrac_cfixed_lr1.25e-5), step21-300:
#   early step21-100: 3.286384633e-6
#   mid   step101-200: 3.370729357e-6
#   late  step201-300: 2.667058466e-6
#   weighted post mean: 3.0953198319e-6
#
# Implementation:
# - Keep the signal_fraction + sign_gate_gamma=1.0 path so the code path and
#   logged split-batch diagnostics match C310/M.
# - Use alpha replay without shuffle to override alpha_t with the D3 schedule.
# - Previous replay logs showed a one-step metric offset: configured start=21
#   appeared at logged step22. Therefore default REPLAY_START_STEP=20 so logged
#   step21-300 should match the 280 replay values exactly.
...
SEED=${SEED:-42}
REPLAY_START_STEP=${REPLAY_START_STEP:-20}
REPLAY_END_STEP=${REPLAY_END_STEP:-299}
ALPHA_REPLAY_PATH=${ALPHA_REPLAY_PATH:-/data/250010176/codes/verl/exp_data/4.27/alpha_replay/d3_piecewise_stage_matched_step21_300.json}

EXP_NAME_BASE=deepseek1.5b_sync_8gpu_piecewise_stage_matched_decay_d3
EXP_NAME=${EXP_NAME:-"${EXP_NAME_BASE}_seed${SEED}"}
CKPT_PREFIX=DeepSeek1.5B-Sync-8gpu-piecewise-stage-matched-decay-d3-seed${SEED}
...
LOG_FILE="${LOG_DIR}/sync_piecewise_stage_matched_decay_d3_seed${SEED}_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.optim.signal_fraction_alpha_replay_path="${ALPHA_REPLAY_PATH}" \
    +actor_rollout_ref.actor.optim.signal_fraction_alpha_replay_shuffle=False \
    +actor_rollout_ref.actor.optim.signal_fraction_alpha_replay_start_step=${REPLAY_START_STEP} \
    +actor_rollout_ref.actor.optim.signal_fraction_alpha_replay_end_step=${REPLAY_END_STEP} \
