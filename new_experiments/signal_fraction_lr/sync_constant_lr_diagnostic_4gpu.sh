...
# ============================================================
# Constant-LR diagnostic run — NO adaptive LR controller
# ============================================================
# Purpose: collect RL gradient-noise source diagnostics under
#   a fixed learning rate.  The signal_fraction code path is
#   kept with sign_gate_gamma=1.0 so that split-batch metrics
#   (r_hat, g_A1_dot_A2, g_dot_positive, …) are still logged,
#   but they do NOT influence the learning rate.
#
# Environment variables:
#   DIAG_LR       — constant learning rate  (required)
#   DIAG_TAG      — short tag for file names (required)
#   SEED          — random seed              (default: 42)
#   RESUME_MODE   — auto / disable           (default: disable)
#
# Example:
#   DIAG_LR=3.10e-6 DIAG_TAG=lr3.1e-6  bash sync_constant_lr_diagnostic.sh
#   DIAG_LR=1e-5    DIAG_TAG=lr1e-5    bash sync_constant_lr_diagnostic.sh
# ============================================================

: "${DIAG_LR:?'Set DIAG_LR (e.g. 3.10e-6)'}"
: "${DIAG_TAG:?'Set DIAG_TAG (e.g. lr3.1e-6)'}"
...
EXP_NAME_BASE="deepseek1.5b_sync_4gpu_diag_constant_${DIAG_TAG}"
EXP_NAME=${EXP_NAME:-"${EXP_NAME_BASE}_seed${SEED}"}
CKPT_PREFIX="DeepSeek1.5B-Sync-4gpu-diag-constant-${DIAG_TAG}-seed${SEED}"
...
LOG_FILE="${LOG_DIR}/sync_diag_constant_${DIAG_TAG}_seed${SEED}_${TIMESTAMP}.log"
