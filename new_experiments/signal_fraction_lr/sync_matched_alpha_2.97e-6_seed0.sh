...
# Phase 2 Group M: matched constant-alpha baseline
# Fixed LR = 2.97e-6 (matches lr1.25e-5 post-handoff mean alpha exactly)
# Purpose: isolate scale effect from r-side contribution; cleanest possible constant baseline.
#
# Uses signal_fraction scheduler with sign_gate_gamma=1.0 (gamma=1 → both g_dot branches
# use the same r_ref → constant LR post-handoff = alpha_base = 2.97e-6).
# This ensures M goes through the same _update_policy_signal_fraction() code path as A and B
# (same split-batch algorithm, same optimizer step count), and also logs g_dot_positive
# so sign distribution can be compared across M/A/B.
...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  echo "Using user-specified checkpoint directory: $CKPTS_DIR"
    ...
    LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/DeepSeek1.5B -name "DeepSeek1.5B-Sync-8gpu-matched-alpha-2.97e-6-seed0-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
    echo "Auto-found existing checkpoint directory: $CKPTS_DIR"
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-Sync-8gpu-matched-alpha-2.97e-6-seed0-${TIMESTAMP}
...
LOG_FILE="${LOG_DIR}/sync_matched_alpha_2.97e-6_seed0_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="deepseek1.5b_sync_8gpu_matched_alpha_2.97e-6_seed0" \
