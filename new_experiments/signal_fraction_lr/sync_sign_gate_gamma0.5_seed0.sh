...
# Phase 2 Group A: sign-gate baseline (gamma=0.5, mean-matched to M/B, warmup-matched to M)
# Compares against M (matched constant 2.97e-6) with equal expected mean alpha AND same warmup.
#
# Mean-matching + warmup-matching via sign_gate_alpha_plus:
#   alpha_base = 2.97e-6  (warmup ramp 0→2.97e-6, same as M)
#   sign_gate_alpha_plus = 3.71e-6 (overrides c_T at handoff → post-handoff g_dot>0 = 3.71e-6)
#
#   Derivation: target_mean = 2.97e-6, gamma = 0.5, p(g_dot>0) ≈ 0.60
#   alpha+ = target_mean / (p + (1-p)*gamma) = 2.97e-6 / 0.80 = 3.7125e-6 ≈ 3.71e-6
#
# At handoff: c_T = 3.71e-6 / max(r_warm, r_boot=0.05) = 3.71e-6/0.05 = 7.42e-5
#   _sign_gate_r_ref = max(r_warm, r_boot) ≈ 0.05
#
#   g_dot > 0  → alpha = 7.42e-5 × 0.05 = 3.71e-6   (above M)
#   g_dot ≤ 0  → alpha = 7.42e-5 × 0.025 = 1.855e-6  (below M)
#
# Expected mean = 0.60×3.71e-6 + 0.40×1.855e-6 = 2.226e-6 + 0.742e-6 = 2.968e-6 ≈ 2.97e-6 ✓
# Warmup 0→2.97e-6 matches M exactly ✓
# Same _update_policy_signal_fraction() code path as M and B ✓  (logs g_dot_positive ✓)
#
# Note: p(g_dot>0) estimate from Phase 1 (55-66%). Check actor/g_dot_positive and
# actor/alpha_t post-handoff mean to confirm actual mean-match.
...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
    ...
    LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/DeepSeek1.5B -name "DeepSeek1.5B-Sync-8gpu-sign-gate-meanmatch-gamma0.5-seed0-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-Sync-8gpu-sign-gate-meanmatch-gamma0.5-seed0-${TIMESTAMP}
  fi
else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-Sync-8gpu-sign-gate-meanmatch-gamma0.5-seed0-${TIMESTAMP}
fi
...
LOG_FILE="${LOG_DIR}/sync_sign_gate_meanmatch_gamma0.5_seed0_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    ++actor_rollout_ref.actor.optim.lr=2.97e-6 \
    ...
    +actor_rollout_ref.actor.optim.signal_fraction_sign_gate_gamma=0.5 \
    ...
    trainer.experiment_name="deepseek1.5b_sync_8gpu_sign_gate_meanmatch_gamma0.5_seed0" \
