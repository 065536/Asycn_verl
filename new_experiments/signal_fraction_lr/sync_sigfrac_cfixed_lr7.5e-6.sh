...
# alpha_base=7.5e-6, r_boot=0.05 → c_fixed = 7.5e-6/0.05 = 1.5e-4
# typical alpha_t = c_fixed * r_typical(0.018) ≈ 2.7e-6
...
if [ "$RESUME_MODE" = "resume_path" ] && [ -n "$RESUME_FROM_PATH" ]; then
  ...
  echo "Using user-specified checkpoint directory: $CKPTS_DIR"
    ...
    LATEST_CKPT=$(find ${VERL_ROOT}/ckpts/DeepSeek1.5B -name "DeepSeek1.5B-Sync-8gpu-sigfrac-cfixed-lr7.5e-6-*" -type d 2>/dev/null | sort | tail -1)
  if [ -n "$LATEST_CKPT" ]; then
    CKPTS_DIR="$LATEST_CKPT"
    TIMESTAMP=$(basename "$CKPTS_DIR" | sed 's/.*-//')
    echo "Auto-found existing checkpoint directory: $CKPTS_DIR"
  else
    CKPTS_DIR=${VERL_ROOT}/ckpts/DeepSeek1.5B/DeepSeek1.5B-Sync-8gpu-sigfrac-cfixed-lr7.5e-6-${TIMESTAMP}
...
LOG_FILE="${LOG_DIR}/sync_sigfrac_cfixed_lr7.5e-6_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    ++actor_rollout_ref.actor.optim.lr=7.5e-6 \
    ...
    trainer.experiment_name="deepseek1.5b_sync_8gpu_sigfrac_cfixed_lr7.5e-6" \
