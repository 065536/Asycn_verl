...
# Baseline 2 (generic): shuffled alpha replay control
#
# Goal:
# - Preserve B's alpha distribution (same alpha multiset as B)
# - Break the per-step alignment coupling by shuffling alpha over time
#
# Required env vars:
# - SEED: training seed (default 42)
# - ALPHA_REPLAY_PATH: path to B alpha replay file (json/csv/txt)
#
# Optional env vars:
# - SHUFFLE_SEED: RNG seed for shuffle (default = SEED)
# - REPLAY_START_STEP: replay start step (default 21)
# - REPLAY_END_STEP: replay end step (default 300)
...
REPLAY_START_STEP=${REPLAY_START_STEP:-21}
REPLAY_END_STEP=${REPLAY_END_STEP:-300}
ALPHA_REPLAY_PATH=${ALPHA_REPLAY_PATH:-""}

if [ -z "$ALPHA_REPLAY_PATH" ]; then
  echo "ALPHA_REPLAY_PATH is required"
  exit 1
fi

EXP_NAME_BASE=deepseek1.5b_sync_8gpu_sigfrac_cfixed_lr1.25e-5_alpha_shuffled
EXP_NAME=${EXP_NAME:-"${EXP_NAME_BASE}_seed${SEED}"}
CKPT_PREFIX=DeepSeek1.5B-Sync-8gpu-sigfrac-cfixed-lr1.25e-5-alpha-shuffled-seed${SEED}
...
LOG_FILE="${LOG_DIR}/sync_sigfrac_cfixed_lr1.25e-5_alpha_shuffled_seed${SEED}_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    ++actor_rollout_ref.actor.optim.signal_fraction_alpha_replay_shuffle=True \
    ++actor_rollout_ref.actor.optim.signal_fraction_alpha_replay_seed=${SHUFFLE_SEED} \
