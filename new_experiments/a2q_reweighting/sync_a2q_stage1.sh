...
# A²Q Hierarchical Gradient Reweighting — Stage 1: fixed-LR ablation
#
# Env vars:
#   A2Q_MODE     — "vanilla" | "rollout_only" | "prompt_only" | "hierarchical"
#   SEED         — random seed (default 42)
#   A2Q_PR       — rollout percentile (default 95)
#   A2Q_PP       — prompt percentile (default 90)
#   A2Q_NORMALIZE — True/False (default True)
...
SEED=${SEED:-42}
A2Q_MODE=${A2Q_MODE:-"vanilla"}
A2Q_PR=${A2Q_PR:-95}
A2Q_PP=${A2Q_PP:-90}
A2Q_LR=${A2Q_LR:-"3.1e-6"}
A2Q_NORMALIZE=${A2Q_NORMALIZE:-"True"}
NCPUS=${NCPUS:-$(nproc)}

if [ "$A2Q_MODE" = "vanilla" ]; then
  A2Q_ENABLED=False
  A2Q_TAG="vanilla"
else
  A2Q_ENABLED=True
  if [ "$A2Q_NORMALIZE" = "False" ]; then
    A2Q_TAG="${A2Q_MODE}_unnorm_pr${A2Q_PR}_pp${A2Q_PP}"
  else
    A2Q_TAG="${A2Q_MODE}_pr${A2Q_PR}_pp${A2Q_PP}"
  fi
fi

EXP_NAME_BASE="deepseek1.5b_sync_8gpu_a2q_${A2Q_TAG}_lr${A2Q_LR}"
EXP_NAME="${EXP_NAME_BASE}_seed${SEED}"
CKPT_PREFIX="DeepSeek1.5B-Sync-8gpu-a2q-${A2Q_TAG}-lr${A2Q_LR}-seed${SEED}"
...
LOG_FILE="${LOG_DIR}/a2q_${A2Q_TAG}_lr${A2Q_LR}_seed${SEED}_${TIMESTAMP}.log"
...
if [ "$IS_HEAD" = "1" ]; then
  $RAY start --head --node-ip-address="$HEAD_IP" --port=6379 --dashboard-port=8265 --num-cpus=${NCPUS}
  ready=0
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    ++algorithm.a2q_reweight_enabled=${A2Q_ENABLED} \
    ++algorithm.a2q_reweight_mode=${A2Q_MODE} \
    ++algorithm.a2q_reweight_percentile_r=${A2Q_PR} \
    ++algorithm.a2q_reweight_percentile_p=${A2Q_PP} \
    ++algorithm.a2q_reweight_ema_beta=0.9 \
    ++algorithm.a2q_reweight_warmup_steps=10 \
    ++algorithm.a2q_reweight_normalize=${A2Q_NORMALIZE} \
    ...
    ++actor_rollout_ref.actor.optim.lr=${A2Q_LR} \
  ...
  $RAY start --address="$HEAD_IP:6379" --num-cpus=${NCPUS}
